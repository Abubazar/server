...
const ser = new WebSocket.Server({port:8082});
var players=[];
var id=""
var gone
...
ser.on("connection", ws => {
    ...
    ws.on("message", message => {
        ...
        if (retr.type=="bio"){
            ...
            ser.clients.forEach(client => {
                if (client.readyState== WebSocket.OPEN){
                    ...
                    client.send(JSON.stringify({name:players[i].name, color:players[i].color, type:"bio"}))
                    }
                }
            })
        }
        if (retr.type=="upd"){
            ser.clients.forEach(client => {
            if (client.readyState== WebSocket.OPEN){
                client.send(JSON.stringify({name:retr.name, x:retr.x, y:retr.y, type:"upd"}))
            }
        })}
        ...
        if(retr.type=="chat"){
            ser.clients.forEach(client => {
                if (client.readyState== WebSocket.OPEN){
                    client.send(JSON.stringify({name:retr.name,msg:retr.msg, type:"chat"}))
                }
            })
        }
